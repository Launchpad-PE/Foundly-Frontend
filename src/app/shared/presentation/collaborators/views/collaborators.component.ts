...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, CollaboratorCardComponent],
...
export class CollaboratorsComponent implements OnInit {
  ...
  get filteredCollaborators(): Collaborator[] {
    return this.allCollaborators.filter(collaborator => {
      const matchesName = this.searchTerm === '' || collaborator.name.toLowerCase().includes(this.searchTerm.toLowerCase());
      const matchesRole = this.filterRole === '' || collaborator.role.toLowerCase().includes(this.filterRole.toLowerCase());
      const matchesSkill = this.filterSkill === '' || collaborator.skills.some(skill => skill.toLowerCase().includes(this.filterSkill.toLowerCase()));
